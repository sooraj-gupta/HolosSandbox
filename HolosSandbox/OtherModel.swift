...
struct OtherModel: View {
    ...
    private func setupAnimation(model: ModelEntity) async {
        ...
        if !animationKeyframes2.isEmpty {
            animationKeyframes2[0].pose = Pose(transforms: startingPoseTransforms)
        }
    ...
    private func findKeyframes(for time: TimeInterval) -> (Keyframe, Keyframe)? {
        ...
        let sortedKeyframes = animationKeyframes2.sorted { $0.time < $1.time }
